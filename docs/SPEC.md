...
- **FR-ENV-9.** While one run is using an environment, another run MUST NOT be let into it. The environments of two projects coincide by default and not by accident: the fingerprint of FR-ENV-1 is built out of the DBMS coordinates and the admin account, so two projects that both leave `MODX_TESTBENCH_DB_NAME` alone get one database, one table prefix and one environment directory, and their runs drop each other's tables. The hold MUST last the whole run rather than the installation alone — the damaging case is two runs that both find the environment ready — and it MUST be released without the cooperation of the process that took it, so that a run killed with `kill -9` frees the environment. A run that is refused MUST be told how to get its own environment (`MODX_TESTBENCH_DB_NAME`) and how to opt out (`MODX_TESTBENCH_ALLOW_CONCURRENT=1`). Subprocesses of the holding run itself are NOT another run: the package spawns them (the build script of a transport package, FR-PKG) and they MUST be let through. Being unable to take the hold — a private directory that cannot be created — MUST NOT fail the run: the guard is a protective measure, like the `0700` of FR-ENV-8, not a success criterion. The lock files MUST NOT accumulate without bound: a file that nobody holds and that has not been touched for a week MAY be deleted, and deleting one MUST NOT be able to hand one environment to two runs — a run whose file is replaced between opening it and locking it MUST NOT consider itself the holder.
...
| `MODX_TESTBENCH_ALLOW_CONCURRENT` | Turns off the guard of FR-ENV-9 and allows several runs over one environment                             | `0`                                                                                                                                 |
